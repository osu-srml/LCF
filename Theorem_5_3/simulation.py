...
def UFtest(loader, model, loss_fn, eta, simdata):
    ...
    for i, data in enumerate(loader):
        ...
        for __ in range(100):
            ...
            y_check = get_y(uy, x_check, simdata.wxy, simdata.wuy, 3 - a)
            x = get_x(ux, a, simdata.wux, simdata.wax)
            y = get_y(uy, x, simdata.wxy, simdata.wuy, a)
            ...
            f_y = get_y(uy, f_x, simdata.wxy, simdata.wuy, a)
            ...
            f_cy = get_y(cuy, f_cx, simdata.wxy, simdata.wuy, 3 - a)
...
def CFtest(loader, model, loss_fn, eta, simdata):
    ...
    for i, data in enumerate(loader):
        ...
        for __ in range(100):
            ...
            f_uy = uy + eta * grad_uy
            f_x = get_x(f_ux, a, simdata.wux, simdata.wax)
            f_y = get_y(f_uy, f_x, simdata.wxy, simdata.wuy, a)
            ...
            f_cx = get_x(f_cux, 3 - a, simdata.wux, simdata.wax)
            f_cy = get_y(f_cuy, f_cx, simdata.wxy, simdata.wuy, 3 - a)
...
def DFtest(loader, model, loss_fn, eta, ratio, simdata):
    ...
    for i, data in enumerate(loader):
        ...
        for __ in range(100):
            ...
            y_check = get_y(u[:, -1].unsqueeze(1), x_check, simdata.wxy, simdata.wuy, 3 - a)
            out = model(y_check, u)
            ...
            cy_check = get_y(cu[:, -1].unsqueeze(1), cx_check, simdata.wxy, simdata.wuy, a)
...
def DFexperiment(loaders, total_epochs, eta, ratio, simdata):
    ...
    model.w1 = torch.tensor(9 / (2 * ratio * eta * simdata.wuy * e ** (-2 / 3))).cuda()
    print("w1 = {}".format(model.w1))
    loss_fn = nn.MSELoss()
    optimizer = optim.Adam(model.parameters(), lr=1e-3, betas=(0.9, 0.99))
