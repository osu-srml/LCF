...
def GPA(S, K, dataset):
    return torch.mm(S, torch.Tensor(dataset.wGS).unsqueeze(1).cuda()) + torch.tensor(dataset.wGK).cuda() * K + dataset.bG

def SAT(S, K, dataset):
    return torch.exp(torch.mm(S, torch.Tensor(dataset.wLS).unsqueeze(1).cuda()) + torch.tensor(dataset.wLK).cuda() * K + dataset.bL)

def FYA(S, K, dataset):
    return torch.mm(S, torch.Tensor(dataset.wFS).unsqueeze(1).cuda()) + torch.tensor(dataset.wFK).cuda() * K
...
def UFtrain(loader, vloader, model, loss_fn, optimizer, total_epochs, eta):
    ...
    for __ in range(total_epochs):
        ...
        for __, data in enumerate(loader):
            ...
            G = data[:, 503].unsqueeze(1).detach().clone().cuda()
            L = data[:, 504].unsqueeze(1).detach().clone().cuda()
            F = data[:, 505].unsqueeze(1).detach().clone().cuda()
            
            out = model(torch.cat([G, L], dim=1))
            loss = loss_fn(out, F)
            loss.backward()
...
def UFtest(loader, model, loss_fn, eta):
    ...
    for i, data in enumerate(loader):
        ...
        G = data[:, 503].unsqueeze(1).detach().clone().cuda()
        L = data[:, 504].unsqueeze(1).detach().clone().cuda()
        target = data[:, 505].unsqueeze(1).detach().clone().cuda()
        ...
        for k_index in range(500):
            ...
            F_check = FYA(1 - S, K, loader.dataset)
            #F = torch.normal(FYA(S, R, K, loader.dataset), 1)
            F = FYA(S, K, loader.dataset)
            ...
            G = GPA(S, K, loader.dataset)
            #L = torch.poisson(SAT(S, R, K, loader.dataset))
            L = SAT(S, K, loader.dataset)
            ...
            f_cF = FYA(1 - S, f_cK, loader.dataset)
...
def UFexperiment(loaders, total_epochs, eta):
    ...
    model = UFdecision(input_dim=2).cuda()
    loss_fn = nn.MSELoss()
    ...
    for i, data in enumerate(loader):
        ...
        for k_index in range(500):
            ...
            with torch.no_grad():
                ...
                F = FYA(S, K, loader.dataset)
                #F_check = torch.normal(FYA(1 - S, R, K, loader.dataset), 1)
                F_check = FYA(1 - S, cK, loader.dataset)
...
def CFexperiment(loaders, total_epochs, eta):
    train_loader, valid_loader, test_loader = loaders[0], loaders[1], loaders[2]
    model = CFdecision(input_dim=1).cuda()
    loss_fn = nn.MSELoss()
    optimizer = optim.Adam(model.parameters(), lr=1e-3, betas=(0.9, 0.99))
...
def DFtrain(loader, vloader, model, loss_fn, optimizer, total_epoch, eta):
    ...
    for __ in range(total_epoch):
        ...
        for __, data in enumerate(loader):
            ...
            S = data[:, 0 : 2].detach().clone().cuda()
            F = data[:, 505].unsqueeze(1).detach().cuda()
            ...
            out = model(F_check, torch.cat([K], dim=1))
            loss = loss_fn(out, F)
            loss.backward()
            running_loss += loss.item()
            optimizer.step()
...
def DFtest(loader, model, loss_fn, eta, ratio):
    ...
    for i, data in enumerate(loader):
        ...
        for K_index in range(500):
            ...
            cF_check = FYA(S, cK, loader.dataset)
            cout = model(cF_check, torch.cat([cK], dim=1))
            cgradient_dummy = torch.ones_like(cout)
            cout.backward(gradient=cgradient_dummy)
            f_cK = cK + eta * cK.grad
            #f_cF = torch.normal(FYA(1 - S, R, f_cK, loader.dataset), 1)
    ...
    for seed in seeds:
        ...
        for i in range(0, 3):
            if i == 0:
                ...
                DFexperiment(loaders, total_epochs, eta, ratio=15)
        print("\n")
